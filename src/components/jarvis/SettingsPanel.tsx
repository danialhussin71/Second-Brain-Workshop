...
export default function SettingsPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <AnimatePresence>{open && <>
    ...
    <motion.section initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 320, damping: 36 }} className="fixed right-0 top-0 z-[61] flex h-full w-full max-w-3xl flex-col border-l border-white/10 bg-[#080b12]/95 shadow-2xl backdrop-blur-2xl">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5">{tab === "brain" && <BrainTab status={status} notice={notice} input={vaultInput} upload={upload} />}{tab === "brand" && <BrandStudio onSaved={setNotice} />}{tab === "carousel" && <CarouselTab />}{tab === "documents" && <DocumentTab notice={notice} upload={upload} onSaved={async (message, ok) => { setNotice(message); if (ok) await refresh(); }} />}</div>
    </motion.section>
  </>}</AnimatePresence>;
}
...
function DocumentTab({ notice, upload, onSaved }: { notice: string; upload: (files: FileList | null, folder?: string) => Promise<void>; onSaved: (message: string, ok: boolean) => Promise<void> }) {
